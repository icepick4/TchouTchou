...
<?php if ($alert == null) {  ?>
    ...
    <?php }else{ ?>
        ...
        <button id="finishButton" class="<? if ($alert['ALERT_STATUS'] != 1 || $user->isStation($_SESSION['user_id'])) echo "disabled"; ?> "><? if ($alert['ALERT_STATUS'] == 1) {
                                                                                                                                                echo ALERT_NOT_FINISHED;
                                                                                                                                            } else {
                                                                                                                                                echo ALERT_FINISHED;
                                                                                                                                            } ?></button>
    </div>

    <iframe width="600" height="450" style="border:0" loading="lazy" allowfullscreen referrerpolicy="no-referrer-when-downgrade" src="https://www.google.com/maps/embed/v1/place?key=AIzaSyAOE0WOjNXmymZ-seKlbzVcSvW-Xaz-sYo
        &q=<?= $coord[0] . "+" . $coord[1] ?>">
    </iframe>
