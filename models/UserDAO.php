...
class UserDAO extends DAO
{
    ...
    {
        $sql = 'SELECT TK.USER_ID, TK.TRAVEL_ID, (SELECT STATION_NAME FROM STATION WHERE STATIOn_ID = START_STATION_ID) AS "START_STATION_NAME", (SELECT STATION_NAME FROM STATION WHERE STATIOn_ID = END_STATION_ID) AS "END_STATION_NAME",TO_CHAR(TR.START_TIME,\'DD/MM/YYYY\') AS "DEPARTURE_DATE", TO_CHAR(TR.START_TIME,\'HH24:MI\') AS "DEPARTURE_TIME", TO_CHAR((SELECT ARRIVAL_TIME FROM ARRIVAL_TO_STATION ATS WHERE STATION_ID = TK.END_STATION_ID AND TRAVEl_ID =TK.TRAVEL_ID),\'HH24:MI\') AS "END_TIME" FROM TICKET TK INNER JOIN TRAVEL TR ON TK.TRAVEL_ID = TR.TRAVEL_ID WHERE TK.USER_ID = :id ORDER BY DEPARTURE_TIME';
        $args = array(':id' => $id,);
