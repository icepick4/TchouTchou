...
class AlertDAO extends DAO{
    ...
    public function getAllCurrentAlertsByTravel($travel_id){
        $sql = 'SELECT * FROM ALERT A INNER JOIN ALERT_TYPE A_T ON A.ALERT_TYPE_ID=A_T.ALERT_TYPE_ID WHERE A.TRAVEL_ID=:travel_id AND ALERT_STATUS=1 ORDER BY ALERT_DATE DESC';
        $args = array(':travel_id' => $travel_id);
        return $this->queryAll($sql, $args);
    }
